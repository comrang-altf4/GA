...
def share_function(d):
    if (d > d_max + a_max):
        return 0
    return 1 - pow((d / (d_max + a_max)), 2)


def compute_share_fitness(index, population):
    ttt = population[index][3]
    ...
    for i in population:
        ...
        if t != 0:
            if i[4] == -1:
                i[4] = individual[4]
            elif individual[4] != i[4]:
                individual[4] = i[4]
                if flag == False:
                    flag = True
                    order = old_order
    individual[3] /= m
    local_max[index] = max(local_max[index], individual[3])
    return individual[3]
...
def getwhitepixel(individual, k, l):
    ...
    # cv2.imwrite("clustercrop.jpg",crop)
    whitepixel = []
    print(crop.shape)
    ...
    print("len whitepixel{}".format(len(whitepixel)))
    clustering = DBSCAN(eps=10, min_samples=100).fit(whitepixel)
    labels = clustering.labels_
    labels_unique = np.unique(labels)
    n_clusters_ = len(labels_unique)
    print(n_clusters_)
    maxx = -10
    for ll in range(n_clusters_):
        my_members = labels == ll
        if len(whitepixel[my_members]) > maxx:
            maxx = len(whitepixel[my_members])
...
for k in range(6):
    ...
    for ll, col in zip(range(n_clusters_), colors):
        ...
        for lll in population[my_members]:
            ...
            cccc += 1
        if len(population[my_members]) > 0:
